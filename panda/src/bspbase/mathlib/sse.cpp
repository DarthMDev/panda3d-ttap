...
float FASTCALL _SSE_VectorNormalize( LVector3& vec )
{
...
void FASTCALL _SSE_VectorNormalizeFast( LVector3& vec )
{
        float ool = _SSE_RSqrtAccurate( FLT_EPSILON + vec[0] * vec[0] + vec[1] * vec[1] + vec[2] * vec[2] );
...
void VectorTransformSSE( const float *in1, const LMatrix4& in2, float *out1 )
{
        nassertv( in1 != out1 );
...
void VectorRotateSSE( const float *in1, const LMatrix4& in2, float *out1 )
{
        nassertv( in1 != out1 );
