...
FORCEINLINE void StoreUnalignedIntSIMD( int32_t *pSIMD, const fltx4 & a )
{
        *( reinterpret_cast<i32x4 *> ( pSIMD ) ) = a;
}
...
// strongly typed -- syntactic castor oil used for typechecking as we transition to SIMD
FORCEINLINE void StoreAligned3SIMD( LVector3 * pSIMD, const fltx4 & a )
{
...
// for the transitional class -- load a 3-by VectorAligned and squash its w component
FORCEINLINE fltx4 LoadAlignedSIMD( const LVector3 & pSIMD )
{
...
FORCEINLINE uint32_t SubFloatConvertToInt( const fltx4 & a, int idx )
{
        return (uint32_t)SubFloat( a, idx );
}

FORCEINLINE uint32_t SubInt( const fltx4 & a, int idx )
{
#ifndef POSIX
        return a.m128_u32[idx];
#else
...
FORCEINLINE uint32_t & SubInt( fltx4 & a, int idx )
{
#ifndef POSIX
        return a.m128_u32[idx];
#else
...
// Load 4 aligned words into a SIMD register
FORCEINLINE i32x4 LoadAlignedIntSIMD( const void * pSIMD )
{
        return _mm_load_ps( reinterpret_cast<const float *>( pSIMD ) );
}
...
// to guarantee an immediate as a parameter to function
// like this.
FORCEINLINE void ConvertStoreAsIntsSIMD( intx4 * pDest, const fltx4 &vSrc )
{
#if 0
...
public:
        ...
        FORCEINLINE void DuplicateVector( LVector3 const &v )			//< set all 4 vectors to the same vector value
        {
                x = ReplicateX4( v[0] );
                y = ReplicateX4( v[1] );
                z = ReplicateX4( v[2] );
        ...
        FORCEINLINE void DuplicateVector( const vec3_t &v )
        {
                x = ReplicateX4( v[0] );
                y = ReplicateX4( v[1] );
                z = ReplicateX4( v[2] );
        ...
        FORCEINLINE fltx4 operator*( LVector3 const &b ) const			//< dot product all 4 vectors with 1 vector
        {
        ...
        FORCEINLINE LVector3 Vec( int idx ) const						//< unpack one of the vectors
        {
        ...
        FORCEINLINE void LoadAndSwizzle( LVector3 const &a, LVector3 const &b, LVector3 const &c, LVector3 const &d )
        {
        ...
        FORCEINLINE void LoadAndSwizzleAligned( const float * a, const float * b, const float * c, const float * d )
        {
#if _X360
        ...
        FORCEINLINE void LoadAndSwizzleAligned( LVector3 const &a, LVector3 const &b, LVector3 const &c, LVector3 const &d )
        {
        ...
        //FORCEINLINE FourVectors( LVector3 const &a, LVector3 const &b, LVector3 const &c, LVector3 const &d )
        //{
...
//-----------------------------------------------------------------------------
FORCEINLINE int BoxOnPlaneSideSIMD( const fltx4& emins, const fltx4& emaxs, const LPlanef &p, float tolerance = 0.f )
{
        fltx4 corners[2];
