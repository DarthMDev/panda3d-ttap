...
enum InterpLatchFlags
{
	...
	INTERPOLATE_LINEAR_ONLY			= 1 << 4, // don't do hermite interpolation
...
// this global keeps the last known server packet tick (to avoid calling
// engine->GetLastTimestamp() all the time)
extern EXPCL_PANDABSP float g_flLastPacketTimestamp;
...
class EXPCL_PANDABSP CInterpolationContext
{
PUBLISHED:
...
extern EXPCL_PANDABSP ConfigVariableDouble cl_extrapolate_amount;

template <class T>
INLINE T ExtrapolateInterpolatedVarType( const T &oldVal, const T &newVal,
					 float divisor,
...
INLINE LVector3 ExtrapolateInterpolatedVarType( const LVector3 &oldVal,
						const LVector3 &newVal,
						float divisor,
						float flExtrapolationAmount )
{
	return TLerp( 1.0f + flExtrapolationAmount * divisor, oldVal, newVal );
}

INLINE float ExtrapolateInterpolatedVarType( const float &oldVal,
					     const float &newVal, float divisor,
					     float flExtrapolationAmount )
{
	return TLerp( 1.0f + flExtrapolationAmount * divisor, oldVal, newVal );
}
...
PUBLISHED:
	...
	virtual bool NoteChanged( float changetime,
				  bool bUpdateLastNetworkedValue ) = 0;
...
struct CInterpolatedVarEntryBase
{
	...
	// This will transfer the data from another varentry.  This is used to avoid
	// allocation pointers can be transferred (only one varentry has a copy), but
...
public:
	...
	void EnsureCapacity( int capSize )
	{
		...
		{
			int newMax =
				m_maxElement + ( ( capSize + m_growSize - 1 ) / m_growSize ) * m_growSize;
			T *pNew = new T[newMax];
			for ( int i = 0; i < m_maxElement; i++ )
			{
				// ------------
				// If you wanted to make this a more generic container you'd probably
				// want this code instead - since FastTransferFrom() is an optimization
				// dependent on types stored here defining this operation.
...
PUBLISHED:
	virtual void Setup( Type *pValue, int type );
...
protected:
	...
	bool GetInterpolationInfo( CInterpolationInfo *pInfo, float currentTime,
				   float interpolation_amount, int *pNoMoreChanges );

	void TimeFixup_Hermite( CInterpolatedVarEntry &fixup,
				CInterpolatedVarEntry *&prev,
				CInterpolatedVarEntry *&start,
				CInterpolatedVarEntry *&end );

	// Force the time between prev and start to be dt (and extend prev out farther
	// if necessary).
	void TimeFixup2_Hermite( CInterpolatedVarEntry &fixup,
				 CInterpolatedVarEntry *&prev,
				 CInterpolatedVarEntry *&start, float dt );

	void _Extrapolate( Type *pOut, CInterpolatedVarEntry *pOld,
			   CInterpolatedVarEntry *pNew, float flDestinationTime,
			   float flMaxExtrapolationAmount );

	void _Interpolate( Type *out, float frac, CInterpolatedVarEntry *start,
			   CInterpolatedVarEntry *end );
	void _Interpolate_Hermite( Type *out, float frac,
				   CInterpolatedVarEntry *pOriginalPrev,
				   CInterpolatedVarEntry *start,
				   CInterpolatedVarEntry *end, bool looping = false );

	void _Derivative_Hermite( Type *out, float frac,
				  CInterpolatedVarEntry *pOriginalPrev,
				  CInterpolatedVarEntry *start,
				  CInterpolatedVarEntry *end );
	void _Derivative_Hermite_SmoothVelocity( Type *out, float frac,
						 CInterpolatedVarEntry *b,
						 CInterpolatedVarEntry *c,
						 CInterpolatedVarEntry *d );
	void _Derivative_Linear( Type *out, CInterpolatedVarEntry *start,
				 CInterpolatedVarEntry *end );
...
protected:
	...
	// Store networked values so when we latch we can detect which values were
	// changed via networking
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::SetInterpolationAmount(
	float seconds )
{
...
template <typename Type, bool IS_ARRAY>
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::NoteChanged(
	float changetime, float interpolation_amount,
	bool bUpdateLastNetworkedValue )
{
	assert( m_pValue );

	// This is a big optimization where it can potentially avoid expensive
	// interpolation involving this variable if it didn't get an actual new value
	// in here.
	bool bRet = true;
	if ( m_VarHistory.Count() )
	{
		if ( memcmp( m_pValue, m_VarHistory[0].GetValue(),
			     sizeof( Type ) * m_nMaxCount ) == 0 )
		{
			bRet = false;
		}
	}
	...
	RemoveOldEntries( now - interpolation_amount - 2.0f );
#else
	// JAY: It doesn't seem like the above code is correct.  This is keeping more
	// than two seconds of history for variables that aren't being interpolated
	// for some reason.  For example, the player model isn't drawn in first
	// person, so the history is only truncated here and will accumulate ~40
	// entries instead of 2 or 3 changing over to the method in Interpolate()
	// means that we always have a 3-sample neighborhood around any data we're
	// going to need.  Unless gpGlobals->curtime is different when samples are
	// added vs. when they are interpolated I can't see this having any ill
	// effects.
	RemoveEntriesPreviousTo( now - interpolation_amount -
				 EXTRA_INTERPOLATION_HISTORY_STORED );
#endif
...
template <typename Type, bool IS_ARRAY>
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::NoteChanged(
	float changetime, bool bUpdateLastNetworkedValue )
{
...
template <typename Type, bool IS_ARRAY>
inline void
CInterpolatedVarArrayBase<Type, IS_ARRAY>::RestoreToLastNetworked()
{
	assert( m_pValue );
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::AddToHead(
	float changeTime, const Type *values, bool bFlushNewer )
{
	...
	if ( bFlushNewer )
	{
		...
		// changeTime is less than a changeTime we added samples during previously.
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::RemoveOldEntries(
	float oldesttime )
{
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::RemoveEntriesPreviousTo(
	float flTime )
{
	for ( int i = 0; i < m_VarHistory.Count(); i++ )
	{
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetInterpolationInfo(
	typename CInterpolatedVarArrayBase<Type, IS_ARRAY>::CInterpolationInfo
	*pInfo,
	float currentTime, float interpolation_amount, int *pNoMoreChanges )
{
	assert( m_pValue );
	...
	for ( int i = 0; i < varHistory.Count(); i++ )
	{
		...
		if ( dt > 0.0001f )
		{
			...
			if ( !( m_fType & INTERPOLATE_LINEAR_ONLY ) &&
			     varHistory.IsIdxValid( oldestindex ) )
			{
			...
			if ( pNoMoreChanges && pInfo->newer == m_VarHistory.Head() )
			{
				...
				{
					if ( !pInfo->m_bHermite ||
					     COMPARE_HISTORY( pInfo->newer, pInfo->oldest ) )
						*pNoMoreChanges = 1;
				}
			}
		}
...
template <typename Type, bool IS_ARRAY>
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetInterpolationInfo(
	float currentTime, int *pNewer, int *pOlder, int *pOldest )
{
	CInterpolationInfo info;
	bool result =
		GetInterpolationInfo( &info, currentTime, m_InterpolationAmount, NULL );
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::DebugInterpolate(
	Type *pOut, float currentTime )
{
	...
	CInterpolationInfo info;
	GetInterpolationInfo( &info, currentTime, interpolation_amount,
			      &noMoreChanges );
	...
	if ( info.m_bHermite )
	{
		...
		_Interpolate_Hermite( pOut, info.frac, &history[info.oldest],
				      &history[info.older], &history[info.newer] );
	}
	else if ( info.newer == info.older )
	{
		// This means the server clock got way behind the client clock. Extrapolate
		// the value here based on its previous velocity (out to a certain amount).
		int realOlder = info.newer + 1;
		if ( CInterpolationContext::IsExtrapolationAllowed() &&
		     IsValidIndex( realOlder ) && history[realOlder].changetime != 0.0 &&
		     interpolation_amount > 0.000001f &&
		     CInterpolationContext::GetLastTimeStamp() <= m_LastNetworkedTime )
		{
			// At this point, we know we're out of data and we have the ability to get
			// a velocity to extrapolate with.
			//
			// However, we only want to extraploate if the server is choking. We don't
			// want to extrapolate if the object legimately stopped moving and the
			// server stopped sending updates for it.
			//
			// The way we know that the server is choking is if we haven't heard
			// ANYTHING from it for a while. The server's update interval should be at
			// least as often as our interpolation amount (otherwise, we wouldn't have
			// the ability to interpolate).
			//
			// So right here, if we see that we haven't gotten any server updates
			// since the last interpolation history update to this entity (and since
			// we're in here, we know that we're out of interpolation data), then we
			// can assume that the server is choking and decide to extrapolate.
			//
			// The End

			// Use the velocity here (extrapolate up to 1/4 of a second).
			_Extrapolate( pOut, &history[realOlder], &history[info.newer],
				      currentTime - interpolation_amount,
...
template <typename Type, bool IS_ARRAY>
inline int CInterpolatedVarArrayBase<Type, IS_ARRAY>::Interpolate(
	float currentTime, float interpolation_amount )
{
	int noMoreChanges = 0;
	...
	{
		// base cast, we have 3 valid sample point
		_Interpolate_Hermite( m_pValue, info.frac, &history[info.oldest],
				      &history[info.older], &history[info.newer] );
	}
	else if ( info.newer == info.older )
	{
		     ...
		     CInterpolationContext::GetLastTimeStamp() <= m_LastNetworkedTime )
		{
			...
			// Use the velocity here (extrapolate up to 1/4 of a second).
			_Extrapolate( m_pValue, &history[realOlder], &history[info.newer],
				      currentTime - interpolation_amount,
				      cl_extrapolate_amount );
		}
		else
		{
			_Interpolate( m_pValue, info.frac, &history[info.older],
				      &history[info.newer] );
		}
	}
	else
	...
	if ( memcmp( backupValues, m_pValue, sizeof( Type ) * m_nMaxCount ) != 0 )
	{
		...
		// This undoes the work that we do in here so if someone is in the debugger,
		// they can find out which variable changed.
	...
	// again. Usually, Interpolate() calls never go backwards in time, but
	// C_BaseAnimating::BecomeRagdollOnClient for one goes slightly back in time
	RemoveEntriesPreviousTo( currentTime - interpolation_amount -
				 EXTRA_INTERPOLATION_HISTORY_STORED );
	return noMoreChanges;
}

template <typename Type, bool IS_ARRAY>
void CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetDerivative(
	Type *pOut, float currentTime )
{
	CInterpolationInfo info;
	...
	if ( info.m_bHermite )
	{
		_Derivative_Hermite( pOut, info.frac, &m_VarHistory[info.oldest],
				     &m_VarHistory[info.older], &m_VarHistory[info.newer] );
	}
	else
	{
		_Derivative_Linear( pOut, &m_VarHistory[info.older],
				    &m_VarHistory[info.newer] );
	}
}

template <typename Type, bool IS_ARRAY>
void CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetDerivative_SmoothVelocity(
	Type *pOut, float currentTime )
{
	CInterpolationInfo info;
	if ( !GetInterpolationInfo( &info, currentTime, m_InterpolationAmount, NULL ) )
	...
	if ( info.m_bHermite )
	{
		_Derivative_Hermite_SmoothVelocity( pOut, info.frac, &history[info.oldest],
						    &history[info.older],
						    &history[info.newer] );
		return;
	}
	else if ( info.newer == info.older &&
		  CInterpolationContext::IsExtrapolationAllowed() )
	{
		// This means the server clock got way behind the client clock. Extrapolate
		// the value here based on its previous velocity (out to a certain amount).
		...
		if ( IsValidIndex( realOlder ) && history[realOlder].changetime != 0.0 )
		{
			...
			// the ability to interpolate).
			//
			// So right here, if we see that we haven't gotten any server updates for
			// a whole interpolation interval, then we know the server is choking.
			//
			// The End
			if ( m_InterpolationAmount > 0.000001f &&
			     CInterpolationContext::GetLastTimeStamp() <=
			     ( currentTime - m_InterpolationAmount ) )
			{
				bExtrapolate = true;
			}
		}
	...
	if ( bExtrapolate )
	{
		...
		diff = clamp( diff, 0.0f, (float)cl_extrapolate_amount.get_value() * 2.0f );
		if ( diff > cl_extrapolate_amount )
		{
			float scale = 1 - ( diff - cl_extrapolate_amount ) /
				cl_extrapolate_amount;
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::Copy(
	IInterpolatedVar *pInSrc )
{
	...
	assert( ( m_fType & ~EXCLUDE_AUTO_INTERPOLATE ) ==
...
template <typename Type, bool IS_ARRAY>
inline const Type &CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetPrev(
	int iArrayIndex ) const
{
	assert( m_pValue );
...
template <typename Type, bool IS_ARRAY>
inline const Type &CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetCurrent(
	int iArrayIndex ) const
{
	assert( m_pValue );
	assert( iArrayIndex >= 0 && iArrayIndex < m_nMaxCount );
...
template <typename Type, bool IS_ARRAY>
inline Type *CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetHistoryValue(
	int index, float &changetime, int iArrayIndex )
{
	assert( iArrayIndex >= 0 && iArrayIndex < m_nMaxCount );
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::SetHistoryValuesForItem(
	int item, Type &value )
{
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::SetLooping(
	bool looping, int iArrayIndex )
{
	assert( iArrayIndex >= 0 && iArrayIndex < m_nMaxCount );
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate(
	Type *out, float frac, CInterpolatedVarEntry *start,
	CInterpolatedVarEntry *end )
{
	assert( start );
	assert( end );
	...
	// Note that LVector3 has a specialization that will do quaternion interpolation
	// here...
	for ( int i = 0; i < m_nMaxCount; i++ )
	{
		if ( m_bLooping[i] )
		...
		{
			out[i] = TLerp( frac, start->GetValue()[i], end->GetValue()[i] );
		}
		Lerp_Clamp( out[i] );
	}
}
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Extrapolate(
	Type *pOut, CInterpolatedVarEntry *pOld, CInterpolatedVarEntry *pNew,
	float flDestinationTime, float flMaxExtrapolationAmount )
{
	if ( fabs( pOld->changetime - pNew->changetime ) < 0.001f ||
	...
	else
	{
		...
		for ( int i = 0; i < m_nMaxCount; i++ )
		{
			pOut[i] = ExtrapolateInterpolatedVarType( pOld->GetValue()[i],
								  pNew->GetValue()[i], divisor,
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::TimeFixup2_Hermite(
	typename CInterpolatedVarArrayBase<Type, IS_ARRAY>::CInterpolatedVarEntry
	&fixup,
	typename CInterpolatedVarArrayBase<Type, IS_ARRAY>::CInterpolatedVarEntry *
	&prev,
	typename CInterpolatedVarArrayBase<Type, IS_ARRAY>::CInterpolatedVarEntry *
	...
	float dt1 )
{
	...
	if ( fabs( dt1 - dt2 ) > 0.0001f && dt2 > 0.0001f )
	{
		...
		for ( int i = 0; i < m_nMaxCount; i++ )
		{
			...
			{
				fixup.GetValue()[i] =
					TLerp( 1 - frac, prev->GetValue()[i], start->GetValue()[i] );
			}
		}
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate_Hermite(
	Type *out, float frac, CInterpolatedVarEntry *prev,
	CInterpolatedVarEntry *start, CInterpolatedVarEntry *end, bool looping )
{
	assert( start );
	assert( end );

	// Disable range checks because we can produce weird values here and it's not
	// an error. After interpolation, we will clamp the values.
	...
	for ( int i = 0; i < m_nMaxCount; i++ )
	{
		// Note that LVector3 has a specialization that will do quaternion
		// interpolation here...
		if ( m_bLooping[i] )
		{
			out[i] = LoopingLerp_Hermite( frac, prev->GetValue()[i],
						      start->GetValue()[i], end->GetValue()[i] );
		}
		else
		{
			out[i] = Lerp_Hermite( frac, prev->GetValue()[i], start->GetValue()[i],
					       end->GetValue()[i] );
		}

		// Clamp the output from interpolation. There are edge cases where something
		// like m_flCycle can get set to a really high or low value when we set it
		// to zero after a really small time interval (the hermite blender will
		// think it's got a really high velocity and skyrocket it off into la-la
		// land).
		Lerp_Clamp( out[i] );
	}
}
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Hermite(
	Type *out, float frac, CInterpolatedVarEntry *prev,
	CInterpolatedVarEntry *start, CInterpolatedVarEntry *end )
{
	assert( start );
	assert( end );
	...
	{
		assert( !m_bLooping[i] );
		out[i] = Derivative_Hermite( frac, prev->GetValue()[i], start->GetValue()[i],
					     end->GetValue()[i] );
		out[i] *= divisor;
	}
}
...
template <typename Type, bool IS_ARRAY>
inline void
CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Hermite_SmoothVelocity(
	Type *out, float frac, CInterpolatedVarEntry *b, CInterpolatedVarEntry *c,
	CInterpolatedVarEntry *d )
{
	CInterpolatedVarEntry fixup;
	fixup.Init( m_nMaxCount );
	...
	{
		Type prevVel =
			( c->GetValue()[i] - b->GetValue()[i] ) / ( c->changetime - b->changetime );
		Type curVel =
			( d->GetValue()[i] - c->GetValue()[i] ) / ( d->changetime - c->changetime );
		out[i] = TLerp( frac, prevVel, curVel );
	}
}

template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Linear(
	Type *out, CInterpolatedVarEntry *start, CInterpolatedVarEntry *end )
{
...
PUBLISHED:
	...
	{
		CInterpolatedVarArrayBase<Type, true>::SetMaxCount( COUNT );
	}
};
...
PUBLISHED:
	...
	{
		CInterpolatedVarArrayBase<Type, false>::SetMaxCount( 1 );
	}
};
